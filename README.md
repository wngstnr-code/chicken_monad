...
- `NEXT_PUBLIC_BACKEND_API_URL` (wajib untuk auth backend + gameplay)

## Status Web3 FE Saat Ini

- Connect wallet + switch chain sudah pakai `wagmi + viem`.
- Frontend sudah bisa auth ke backend lewat SIWE dan menyimpan cookie session.
- Halaman `/deposit` sudah menjalankan flow on-chain: `allowance -> approve -> deposit -> withdraw`.
- Halaman `/play` sekarang sudah terhubung ke backend game gateway dan kontrak live:
  `game:start -> startSession -> game:cashout/crash -> settleWithSignature`.
...
- `frontend/app/deposit/page.tsx`: flow approve + deposit + withdraw USDC
...
- `frontend/components/game/GameBridgeClient.tsx`: bridge game ke backend + smart contract
